...
import { Play, Pause, SkipBack, SkipForward, Shuffle, Repeat, Volume2, Heart, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
function MusicPlayer({ isPlaying }: { isPlaying: boolean }) {
  ...
  const {togglelikeSong, checkisLiked,fetchlikedSongs,likedSongs} = useMusicStore();
  ...
  useEffect(()=>{
    const checklike = async()=>{
      if(currentSong){
      const preliked = await checkisLiked(currentSong._id);
  ...
  const handleLike = async (songId:string | undefined) => {
    ...
    try {
      console.log('music player',songId);
